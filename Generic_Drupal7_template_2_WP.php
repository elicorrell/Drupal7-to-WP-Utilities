...
Although this uses Drupal 7 as the source, it should be simple enough to swap this with another CMS database.
*/
echo '<h1>Generic Drupal 7 template Import or Update to WP post type</h1>';
// let see those nasty errors, Dude.
...
// Get template pages
// counters
$dcount=0; // Old CMS posts
$wpcount=0; // inserted posts
$wpucount =0; // updated posts
//
// set template name
...
while ($row = $drupal->fetch_assoc()) {
   ...
   // query a failed
	...
	// (sample) get Introduction -  - field_data_introduction, introduction_value
...
// Gather post data.
// don't load until you are absolutely ready!
$insert_check = 'false'; // set to true to add/update posts 
    
if ($insert_check == 'true'){
    // check if post type exists, then update, otherwise add
    /* The update option allows you to build WordPress functionality on your posts before golive. Instead of deleting and insert all the posts again, this ensures your content will be up-to-date.  
    */
...
echo('<br>DONE - total Drupal '.$drupal_template.': '.$dcount.'<br>');
echo('<br>DONE - total WordPress '.$WP_post_type.' Inserted: '.$wpcount.'<br>');
echo('<br>DONE - total WordPress '.$WP_post_type.' Updated: '.$wpucount.'<br>');
$myconn -> close();
//
?>
</body>
